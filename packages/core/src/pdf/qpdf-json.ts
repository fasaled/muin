...
export function parseQpdfJson(input: unknown, maxObjects?: number): PdfStructure {
  ...
  const objectsBag = objectsRaw as Record<string, unknown>;
  let objectCount = 0;
  for (const key of Object.keys(objectsBag)) {
    if (key !== "trailer") objectCount++;
  }
  assertObjectCount(objectCount, maxObjects);

  const objects: Record<string, PdfIndirectObject> = {};
  let trailer: PdfDict | undefined;

  for (const [key, raw] of Object.entries(objectsBag)) {
    if (key === "trailer") {
