...
Package versions stay in lockstep (`packages/core`, `packages/cli`, and `packages/core/src/version.ts`). Bump all three together, push to `main`, then create the GitHub Release locally (tag + notes; GitHub attaches source zip/tar.gz):

```bash
gh release create v0.1.0 --title "Muin 0.1.0" --generate-notes --target main
```

Do not attach npm tarballs. npm publish of `@fasaled/muin` is a separate step (`npm publish --access public` from `packages/cli`).
